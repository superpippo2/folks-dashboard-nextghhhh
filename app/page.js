...
// ── POOLS ─────────────────────────────────────────────────────────────────────
const CDN = "https://asa-list.tinyman.org/assets";
const MAIN_POOLS = [
  { name:"ALGO",   color:"#00B2FF", icon:"A",  logo:`${CDN}/0/icon.png`,          depositAPY:3.82, varBorrowAPY:5.41, stblBorrowAPY:6.20, totalDeposits:48200000, totalBorrow:29100000, collateralCap:55000000, borrowCap:40000000, price:0.158 },
  { name:"xALGO",  color:"#00C896", icon:"x",  logo:"/xalgo.png",                  depositAPY:2.97, varBorrowAPY:4.88, stblBorrowAPY:5.50, totalDeposits:9200000,  totalBorrow:4100000,  collateralCap:11000000, borrowCap:6000000,  price:0.162 },
  { name:"tALGO",  color:"#9B59B6", icon:"t",  logo:"/talgo.png",                  depositAPY:2.50, varBorrowAPY:4.50, stblBorrowAPY:5.20, totalDeposits:5000000,  totalBorrow:2000000,  collateralCap:8000000,  borrowCap:4000000,  price:0.162 },
  { name:"USDC",   color:"#2775CA", icon:"$",  logo:`${CDN}/31566704/icon.png`,    depositAPY:5.14, varBorrowAPY:7.23, stblBorrowAPY:8.10, totalDeposits:12400000, totalBorrow:9800000,  collateralCap:14000000, borrowCap:11000000, price:1.0   },
  { name:"goBTC",  color:"#F7931A", icon:"₿",  logo:`${CDN}/386192725/icon.png`,   depositAPY:0.42, varBorrowAPY:1.88, stblBorrowAPY:2.40, totalDeposits:48,       totalBorrow:21,       collateralCap:70,       borrowCap:40,       price:65200 },
  { name:"goETH",  color:"#8C9EFF", icon:"Ξ",  logo:`${CDN}/386195940/icon.png`,   depositAPY:0.61, varBorrowAPY:2.14, stblBorrowAPY:2.80, totalDeposits:820,      totalBorrow:390,      collateralCap:1100,     borrowCap:700,      price:3410  },
  { name:"WBTC",   color:"#FF9900", icon:"₿",  logo:"/wbtc.png",                   depositAPY:0.38, varBorrowAPY:1.72, stblBorrowAPY:2.20, totalDeposits:32,       totalBorrow:12,       collateralCap:50,       borrowCap:30,       price:65300 },
...
const ISOLATED_POOLS = [
  ...
  { name:"ISO USDC",  color:"#2775CA", icon:"$", logo:`${CDN}/31566704/icon.png`,  depositAPY:3.80, varBorrowAPY:5.90, stblBorrowAPY:6.80, totalDeposits:4100000, totalBorrow:2900000, collateralCap:5000000, borrowCap:3500000, price:1.0   },
  { name:"ISO TINY",  color:"#E040FB", icon:"T", logo:"/tiny.png", depositAPY:1.20, varBorrowAPY:3.40, stblBorrowAPY:4.10, totalDeposits:980000,  totalBorrow:320000,  collateralCap:1200000, borrowCap:600000,  price:0.048 },
  { name:"ISO FOLKS", color:"#FF6B6B", icon:"F", logo:"/folks.png",               depositAPY:1.80, varBorrowAPY:3.90, stblBorrowAPY:4.70, totalDeposits:2100000, totalBorrow:850000,  collateralCap:2500000, borrowCap:1200000, price:0.12  },
];
    ...
    <div ref={ref} style={{
      ...
      <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap"}}>
        {[
          ...
          <button key={s.label} onClick={()=>{ setTo(toDateStr(now)); setFrom(toDateStr(now - s.d*86400000)); }}
            style={{background:"none",border:"1px solid #162032",color:"#3A5270",padding:"3px 9px",borderRadius:5,fontSize:10,fontFamily:"'DM Sans',sans-serif",cursor:"pointer"}}>
            {s.label}
          </button>
        ))}
      </div>
      <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
        <button onClick={onClose} style={{background:"none",border:"1px solid #162032",color:"#3A5270",padding:"6px 14px",borderRadius:7,fontSize:12,fontFamily:"'DM Sans',sans-serif",cursor:"pointer"}}>Cancel</button>
        <button onClick={()=>valid&&onApply(fromDateStr(from),fromDateStr(to))} disabled={!valid}
          style={{background:valid?"linear-gradient(135deg,#0AAFE6,#0055CC)":"#0A1628",border:"none",color:valid?"#fff":"#2E4A68",padding:"6px 16px",borderRadius:7,fontSize:12,fontWeight:600,fontFamily:"'DM Sans',sans-serif",cursor:valid?"pointer":"default",transition:"all .15s"}}>
          Apply
        </button>
      </div>
    </div>
    ...
    <div style={{position:"relative",display:"flex",gap:0}}>
      <div style={{display:"flex",background:"#060A14",border:"1px solid #0C1628",borderRadius:8,overflow:"visible"}}>
        {RANGES.map((r, i) => {
          ...
          return (
            <button key={r.key}
              onClick={() => { if (isCustom) { setOpen(v=>!v); } else { onChange(r.key); setOpen(false); } }}
              style={{
                background: isActive ? "#0D1A2E" : "none",
                border: "none", borderRight: !isLast ? "1px solid #0C1628" : "none",
                borderRadius: isCustom ? "0 8px 8px 0" : 0,
                color: isActive ? "#0AAFE6" : "#3A5270",
                padding: "5px 11px", fontSize: 11, fontWeight: isActive ? 600 : 400,
                fontFamily: "'DM Sans',sans-serif", cursor: "pointer", transition: "all .15s", minWidth: 36,
      ...
      {rangeKey === "custom" && !open && (
        <div style={{position:"absolute",top:"calc(100% + 4px)",right:0,fontSize:10,color:"#0AAFE6",fontFamily:"'DM Mono',monospace",background:"#060A14",border:"1px solid #0C1628",borderRadius:5,padding:"2px 8px",whiteSpace:"nowrap",zIndex:10}}>
      ...
      {open && (
        <DateRangePopover fromMs={fromMs} toMs={toMs}
...
const AssetBtn = ({pool,active,onClick}) => (
  <button onClick={onClick} style={{
    ...
    {pool.logo
      ? <img src={pool.logo} alt={pool.name} width={20} height={20} style={{borderRadius:"50%",objectFit:"contain"}} onError={e=>{e.currentTarget.style.display="none";e.currentTarget.nextSibling.style.display="inline";}} />
      : null}
    <span style={{fontSize:14,fontWeight:700,display:pool.logo?"none":"inline"}}>{pool.icon}</span>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    fetch("/api/history")
      .then(r => r.json())
      .then(json => { if (json.success) setHistoryData(json.results); })
      .catch(console.error);
  }, []);
  ...
  const [amtRangeKey, setAmtRangeKey]   = useState("1Y");
  const [amtFrom, setAmtFrom]           = useState(NOW - 365*86400000);
  ...
  const pool = useMemo(() => {
    ...
    const live = liveData.find(d => d.key === KEY_MAP[selectedName]);
    if (!live) return base;
  ...
  const apyHistory = useMemo(() => {
    const key = KEY_MAP[selectedName];
    ...
    if (basePoints.length < 2) return genHistory(pool, apyFrom, apyTo);
    // linear interpolation to fill daily gaps
    const interpolated = [];
    for (let i = 0; i < basePoints.length - 1; i++) {
      const a = basePoints[i], b = basePoints[i + 1];
      const t0 = new Date(a.date).getTime(), t1 = new Date(b.date).getTime();
    ...
    interpolated.push(basePoints.at(-1)); // add final point
    // filter to requested range
    const filtered = interpolated.filter(d => {
      const t = new Date(d.date).getTime();
      return t >= apyFrom && t <= apyTo;
    });
    if (filtered.length === 0) return genHistory(pool, apyFrom, apyTo);
    const days = Math.round((apyTo - apyFrom) / 86400000);
    const granularity = getGranularity(days);
    return filtered.map(d => ({
      label: fmtTick(new Date(d.date), granularity),
      depositAPY:    d.depositAPY    || 0,
      varBorrowAPY:  d.varBorrowAPY  || 0,
      stblBorrowAPY: d.stblBorrowAPY || 0,
    }));
  }, [selectedName, apyFrom, apyTo, historyData, pool]);

  const amtHistory = useMemo(() => {
    const key = KEY_MAP[selectedName];
    const raw = historyData?.[key];
    if (!raw || raw.length === 0) return genHistory(pool, amtFrom, amtTo);
    const today = new Date().toISOString().slice(0, 10);
    const livePoint = pool.totalDeposits != null ? { date: today, totalDeposits: pool.totalDeposits, totalVariableBorrow: pool.totalBorrow } : null;
    const withLive = [...raw.filter(d => d.date !== today), ...(livePoint ? [livePoint] : [])];
    if (withLive.length === 0) return genHistory(pool, amtFrom, amtTo);
    // anchor before range if needed
    const anchor = withLive.filter(d => new Date(d.date).getTime() < amtFrom).at(-1);
    const basePoints = anchor ? [anchor, ...withLive.filter(d => new Date(d.date).getTime() >= amtFrom)] : withLive;
    if (basePoints.length < 2) return genHistory(pool, amtFrom, amtTo);
    // linear interpolation to fill daily gaps
    const interpolated = [];
    for (let i = 0; i < basePoints.length - 1; i++) {
      const a = basePoints[i], b = basePoints[i + 1];
      ...
      for (let d = 0; d < nDays; d++) {
        ...
        interpolated.push({
          date: new Date(t0 + d * 86400000).toISOString().slice(0, 10),
          totalDeposits:       a.totalDeposits       + (b.totalDeposits       - a.totalDeposits)       * frac,
          totalVariableBorrow: a.totalVariableBorrow != null && b.totalVariableBorrow != null
            ? a.totalVariableBorrow + (b.totalVariableBorrow - a.totalVariableBorrow) * frac
            : (a.totalVariableBorrow ?? b.totalVariableBorrow ?? 0),
        });
      }
    }
    interpolated.push(basePoints.at(-1));
    const filtered = interpolated.filter(d => {
      const t = new Date(d.date).getTime();
      return t >= amtFrom && t <= amtTo;
    });
    if (filtered.length === 0) return genHistory(pool, amtFrom, amtTo);
    const days = Math.round((amtTo - amtFrom) / 86400000);
    const granularity = getGranularity(days);
    return filtered.map(d => ({
      label: fmtTick(new Date(d.date), granularity),
      depositAmt: d.totalDeposits,
      borrowAmt:  d.totalVariableBorrow || 0,
      depositUSD: d.totalDeposits * (pool.price || 1),
      borrowUSD:  (d.totalVariableBorrow || 0) * (pool.price || 1),
    }));
      ...
      <header style={{borderBottom:"1px solid #0C1628",background:"rgba(6,10,20,0.97)",backdropFilter:"blur(10px)",position:"sticky",top:0,zIndex:100}}>
        <div style={{maxWidth:1240,margin:"0 auto",padding:"0 20px",display:"flex",alignItems:"center",justifyContent:"space-between",height:52}}>
          <div style={{display:"flex",alignItems:"center",gap:9}}>
            ...
            <span style={{fontSize:13,fontWeight:600,letterSpacing:-.2,cursor:"pointer"}} onClick={()=>setEasterEgg(v=>!v)}>Folks Finance</span>
      ...
      {easterEgg && (
        <div style={{position:"fixed",top:0,left:0,width:"100vw",height:"100vh",background:"rgba(0,0,0,0.7)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>{setEasterEgg(false);setEasterInput("");}}>
          <div style={{background:"#080E1C",border:"1px solid #0C1628",borderRadius:16,padding:"32px 40px",textAlign:"center",fontFamily:"'DM Sans',sans-serif"}} onClick={e=>e.stopPropagation()}>
            ...
            <input autoFocus value={easterInput} onChange={e=>setEasterInput(e.target.value)} placeholder="Type your answer..."
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
            <div style={{display:"flex",alignItems:"center",gap:11}}>
              <div style={{width:42,height:42,borderRadius:12,background:`${pool.color}16`,border:`1px solid ${pool.color}25`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:17,fontWeight:700,color:pool.color}}>
                {pool.logo
                  ? <img src={pool.logo} alt={pool.name} width={26} height={26} style={{borderRadius:"50%",objectFit:"contain"}} onError={e=>{e.currentTarget.style.display="none";e.currentTarget.nextSibling.style.display="inline";}} />
                  : null}
                <span style={{display:pool.logo?"none":"inline"}}>{pool.icon}</span>
              </div>
              <div>
          ...
          <Card style={{overflow:"visible"}}>
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:14,gap:12,flexWrap:"wrap"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                ...
                <RangePicker rangeKey={apyRangeKey} fromMs={apyFrom} toMs={apyTo} onChange={handleApyRange} onCustomApply={(f,t)=>{ setApyFrom(f); setApyTo(t); }}/>
              </div>
            </div>
          ...
          <Card style={{overflow:"visible"}}>
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:14,gap:12,flexWrap:"wrap"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                ...
                <div style={{width:1,height:20,background:"#0C1628"}}/>
                <RangePicker rangeKey={amtRangeKey} fromMs={amtFrom} toMs={amtTo} onChange={handleAmtRange} onCustomApply={(f,t)=>{ setAmtFrom(f); setAmtTo(t); }}/>
              </div>
            </div>
